# database/models.py
#
# DDL constants — two variants:
#   CREATE_*          — SQLite syntax
#   PG_CREATE_*       — PostgreSQL syntax
#
# Use get_ddl_set(is_postgres) at runtime to pick the right set.
...
# ── PostgreSQL DDL ──────────────────────────────────────────────────────────
# Uses SERIAL PRIMARY KEY + BIGINT PRIMARY KEY where SQLite uses AUTOINCREMENT

PG_CREATE_TOKENS_TABLE = """
CREATE TABLE IF NOT EXISTS tokens (
    address TEXT PRIMARY KEY,
    symbol TEXT NOT NULL,
    name TEXT NOT NULL,
...
PG_CREATE_WALLET_ANALYSES_TABLE = """
CREATE TABLE IF NOT EXISTS wallet_analyses (
    id                SERIAL PRIMARY KEY,
    token_address     TEXT NOT NULL,
    creator_wallet    TEXT,
...
PG_CREATE_SOCIAL_SIGNALS_TABLE = """
CREATE TABLE IF NOT EXISTS social_signals (
    id                  SERIAL PRIMARY KEY,
    token_address       TEXT NOT NULL,
    mention_count_1h    INTEGER,
...
PG_CREATE_DETECTED_POOLS_TABLE = """
CREATE TABLE IF NOT EXISTS detected_pools (
    id           SERIAL PRIMARY KEY,
    mint_address TEXT NOT NULL,
    dex_name     TEXT NOT NULL,
...
PG_CREATE_HOLDER_VELOCITY_TABLE = """
CREATE TABLE IF NOT EXISTS holder_velocity_snapshots (
    id                    SERIAL PRIMARY KEY,
    token_address         TEXT NOT NULL,
    snapshot_time         TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
PG_CREATE_LIQUIDITY_GROWTH_TABLE = """
CREATE TABLE IF NOT EXISTS liquidity_growth_results (
    id                      SERIAL PRIMARY KEY,
    token_address           TEXT    NOT NULL,
    candle_count            INTEGER DEFAULT 0,
...
PG_CREATE_CIRCUIT_BREAKER_TABLE = """
CREATE TABLE IF NOT EXISTS circuit_breaker_state (
    id                  INTEGER PRIMARY KEY DEFAULT 1,
    level               TEXT    NOT NULL DEFAULT 'OK',
    reason              TEXT    NOT NULL DEFAULT '',
...
def get_ddl(is_postgres: bool) -> dict[str, str]:
    """Return the full DDL dict for the active backend."""
    if is_postgres:
        return {
            "tokens": PG_CREATE_TOKENS_TABLE,
            "signals": PG_CREATE_SIGNALS_TABLE,
            "scans": PG_CREATE_SCANS_TABLE,
            "wallet_analyses": PG_CREATE_WALLET_ANALYSES_TABLE,
            "social_signals": PG_CREATE_SOCIAL_SIGNALS_TABLE,
            "rug_analyses": PG_CREATE_RUG_ANALYSES_TABLE,
            "detected_pools": PG_CREATE_DETECTED_POOLS_TABLE,
            "holder_velocity": PG_CREATE_HOLDER_VELOCITY_TABLE,
            "first_buyer": PG_CREATE_FIRST_BUYER_TABLE,
            "tx_pattern": PG_CREATE_TX_PATTERN_TABLE,
            "liquidity_growth": PG_CREATE_LIQUIDITY_GROWTH_TABLE,
            "cross_dex": PG_CREATE_CROSS_DEX_TABLE,
            "circuit_breaker": PG_CREATE_CIRCUIT_BREAKER_TABLE,
        }
    return {
        "tokens": CREATE_TOKENS_TABLE,
        "signals": CREATE_SIGNALS_TABLE,
        "scans": CREATE_SCANS_TABLE,
        "wallet_analyses": CREATE_WALLET_ANALYSES_TABLE,
        "social_signals": CREATE_SOCIAL_SIGNALS_TABLE,
        "rug_analyses": CREATE_RUG_ANALYSES_TABLE,
        "detected_pools": CREATE_DETECTED_POOLS_TABLE,
        "holder_velocity": CREATE_HOLDER_VELOCITY_TABLE,
        "first_buyer": CREATE_FIRST_BUYER_TABLE,
        "tx_pattern": CREATE_TX_PATTERN_TABLE,
        "liquidity_growth": CREATE_LIQUIDITY_GROWTH_TABLE,
        "cross_dex": CREATE_CROSS_DEX_TABLE,
        "circuit_breaker": CREATE_CIRCUIT_BREAKER_TABLE,
    }
